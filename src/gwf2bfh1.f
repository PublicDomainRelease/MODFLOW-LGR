...
C-----VERSION 1.1 25OCTOBER2006 GWF2BFH1AR      
...
C     ------------------------------------------------------------------
     ...
     2                      NPCEND,IUBC,NGRIDS,NCPP,BTEXT,IBSHRD,IBPFLG,
     3                      KLAY,IROW,JCOL,KPLAY,IPROW,JPCOL,NPINDX,
     4                      BFLUX,BHEAD,BFLUXCHK,BHEADCHK,VCB 
C
      LOGICAL             ::LOP
      CHARACTER*200 LINE
...
C     ------------------------------------------------------------------
     ...
     &         NPRBEG,NPCBEG,NPLEND,NPREND,NPCEND,IUBC,NGRIDS,NCPP,
     &         BTEXT,VCB(4))
      
C1------PRINT A MESSAGE IDENTIFYING BFH PACKAGE
      WRITE(IOUT,500)IN
  500 FORMAT(1X,/1X,'BFH -- BOUNDARY FLOW AND HEAD PACKAGE',
     &    ', VERSION 1.1, 11/10/2006',/8X,'INPUT READ FROM UNIT ',I4)
...
C4------ALLOCATE BOUNDARY CONDITION ARRARYS. 
     ...
     &           JPCOL(NBNODES),NPINDX(NBNODES),IBSHRD(NBNODES))  
...
C
      ...
      IF(BFHDAT(IGRID)%ISCHILD .GE. 0)THEN
        ...
        DEALLOCATE(BFHDAT(IGRID)%IBSHRD)
        IF(BFHDAT(IGRID)%IUBC .GT. 0) DEALLOCATE(BFHDAT(IGRID)%BFLUXCHK)
      ELSE
        DEALLOCATE(BFHDAT(IGRID)%IBPFLG)
        IF(BFHDAT(IGRID)%IUBC .GT. 0) DEALLOCATE(BFHDAT(IGRID)%BHEADCHK)
      ENDIF
...
C     ------------------------------------------------------------------
     ...
     &                      JPCOL,NPINDX,IBSHRD,IUBC,NGRIDS,IBPFLG
      CHARACTER*200 LINE
...
C
      ELSE
        DO N=1,NBNODES
          READ(IN,*) KLAY(N),IROW(N),JCOL(N),IBB
          IBOUND(JCOL(N),IROW(N),KLAY(N))=IBB
        ENDDO
C
...
C3A-----THE INTERIOR CELLS BETWEEN INTERFACE CELLS AND ZERO THEM OUT.
        ...
        DO LG = 1,NGRIDS-1
          DO K = NPLBEG,NPLEND
            DO I = NPRBEG,NPREND
              J= NPCBEG
              DO WHILE (J .LT. NPCEND)
                J=J+1
                IF(IBOUND(J-1,I,K) .EQ. IBPFLG(LG) .AND.
     &             IBOUND(J,I,K) .NE. IBPFLG(LG)) THEN 
C-------FOUND A CELL NEXT TO AN INTERFACE CELL.  FIND OUT IF IT IS AN
C-------INTERIOR CELL BY SEARCHING FOR ANOTHER INTERFACE CELL. 
C-------ZERO OUT VALUES BETWEEN PAIRS OF INTERFACE CELLS (JBEG:JEND)
                  JBEG=J
                  JEND=J-1
                  DO JJ=J+1,NPCEND 
                    IF(IBOUND(JJ,I,K) .EQ. IBPFLG(LG))THEN 
                      JEND=JJ - 1
                      EXIT
                    ENDIF
                  ENDDO     
                  DO JJ=JBEG,JEND
                    IBOUND(JJ,I,K)=0
                    HNEW(JJ,I,K) = HNF
                    IF(K.NE.NLAY) CV(JJ,I,K)=ZERO
                    IF(K.NE.1) CV(JJ,I,K-1)=ZERO
                    CC(JJ,I,K)=ZERO
                    IF(I.NE.1) CC(JJ,I-1,K)=ZERO
                    CR(JJ,I,K)=ZERO
                    IF(JJ.NE.1) CR(JJ-1,I,K)=ZERO
                  ENDDO
C-------END OF INTERIOR WAS REACHED. MOVE TO NEXT COLUMN SO WE DON'T
C-------COUNT THIS CELL AS THE START OF ANOTHER INTERFACE.  THAT IS,
C-------THERE NEEDS TO BE AT LEAST ONE PARENT CELL BETWEEN AN INTERFACE.
                  J=JEND+2
                ENDIF
              ENDDO
            ENDDO
...
C3B-----IF TRANSIENT, THEN ADJUST STORAGES
        IF(ITRSS .NE. 0) THEN
          DO LG=1,NGRIDS-1
            CALL SGWF2BFH1FMPS(IUBCF,IULPF,IUHUF,IBPFLG(LG)) 
          ENDDO
        ENDIF
...
C-----VERSION 1.0 15FEBRUARY SGWF2BFH1FMPS      
      SUBROUTINE SGWF2BFH1FMPS(IUBCF,IULPF,IUHUF,IBFLG)
C     ******************************************************************
...
C5
      ...
      END
C***********************************************************************
C-----VERSION 1.0 26OCTORBER GWF2BFH1FM
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE GWFBFHMODULE,ONLY:ISCHILD,NGRIDS,IBPFLG
C     ------------------------------------------------------------------
      CALL SGWF2BFH1PNT(IGRID)
C1
C1------ADJUST CONDUCTANCES DEPENDING ON WHETHER A PARENT OR CHILD GRID
      ...
      ELSE
        DO LG=1,NGRIDS-1
          CALL SGWF2BFH1FMPBC(KPER,KSTP,KITER,IUBCF,IBPFLG(LG))
        ENDDO
...
C-----VERSION 1.0 15FEBRUARY SGWF2BFH1FMPBC      
      SUBROUTINE SGWF2BFH1FMPBC(KPER,KSTP,KITER,IUBCF,IBFLG)
C
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     1                      ISSFLG,HNEW,CR,CC,CV
      ...
      USE GWFBFHMODULE,ONLY:NBNODES,JCOL,IROW,KLAY,NPINDX,IBSHRD
...
C10-----IF TRANSIENT, ACCOUNT FOR FLUX TO/FROM STORAGE 
        IF(ISS .EQ. 0 .AND. IBSHRD(N) .EQ. 1)THEN
