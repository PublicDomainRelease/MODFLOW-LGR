...
C
      ...
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 GWF2LGR1AR      
      SUBROUTINE GWF2LGR1AR(INLGR,FNAME,IGRID)
C     ******************************************************************
...
C     ------------------------------------------------------------------
     ...
     4                      HDIFFM,FDIFFM,NCPPL,NODEH,NODEF,KPLC,IPLC,
     5                      JPLC,NPINDX,ICBOUND,HOLDC,CCC,CCR,CCV,PFLUX,
     6                      PFLUXOLD,VCB 
C
      INTEGER, SAVE:: NLAYP
      CHARACTER*200 LINE, FNAME 
...
C     ------------------------------------------------------------------
     ...
     1         NPL,IBOTFLG,ISHFLG,IBFLG,IUPBHSV,IUCBHSV,IUPBFSV,IUCBFSV,
     2         MXLGRITER,IOUTLGR,RELAXH,RELAXF,HCLOSELGR,FCLOSELGR,
     3         HDIFFM,FDIFFM)      
C1------PRINT A MESSAGE IDENTIFYING LGR PACKAGE
      WRITE(IOUT,500)IGRID,TRIM(FNAME)
  500 FORMAT(1X,/1X,'LGR -- LOCAL GRID REFINEMENT',
     &    ', VERSION 1.0, 02/15/2006',/8X,'INPUT READ FOR MODEL ',
...
C4B1----PRINT COMMENTS
        ...
        IF(RELAXF .LE. ZERO .OR. RELAXF .LE. ZERO)THEN
...
C4D1----CHECK REFINEMENT RATIOS AND GRID DISCRETIZATION
     ...
     &                 ' NPCEND - NPCBEG'
        ...
        ENDIF
C
C4D2----CHECK VERTICAL REFINEMENT (NLAYP IS SAVED FROM PREVIOUS GRID)
        IBOTFLG = 1
        IF(NPLBEG .EQ. NPLEND .OR.
     &     NPLEND .EQ. NLAYP) IBOTFLG=0
        NSUM = 0
        IF(NPL .GT. 1)THEN
          DO K=1,NPL
            NSUM = NSUM + NCPPL(K) 
...
C SET NLAYP TO NLAY SO IT WILL BE SAVED FROM THE PREVIOUS GRID
C NOTE: THIS ONLY WORKS FOR SEQUENTIAL GRIDS!!!
      NLAYP = NLAY
C     
...
C     ------------------------------------------------------------------
      ...
      USE LGRMODULE,   ONLY:NCPP,NCPPL,NPL,KPLC,IPLC,JPLC,NPINDX,NPCBEG,
     1                      NPRBEG,NPCEND,NPREND,IBFLG,ISCHILD
C     ------------------------------------------------------------------
...
C
      ...
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 GWF2LGR1INITP
      SUBROUTINE GWF2LGR1INITP(IGRID,KPER,KSTP,LGRITER,NPCBEG,NPRBEG,
     1                         NPLBEG,NPCEND,NPREND,NPLEND,IBOTFLG,
     2                         IBFLG,ISHFLG,MXLGRITER)
C     ******************************************************************
...
C3
      ...
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 GWF2LGR1FMPBS      
...
C2------ITERATION OF THE SIMULATION.  FIRST ITERATION IS A FULL GRID.
      IF(LGRITER .EQ. 2 .AND. KSTP .EQ. 1 .AND. KPER .EQ.1)THEN
        CALL SGWF2LGR1FMPS(IUBCF,IULPF,IUHUF,IBFLG)
      ENDIF
C3
      RETURN
      END
...
C     ------------------------------------------------------------------
     ...
     1                      KPLC,IPLC,JPLC,IBOTFLG,IBFLG,PFLUX,PFLUXOLD,
     2                      FDIFFM,RELAXF,NODEF,MXLGRITER  
...
C
C13-----IF TRANSIENT, ACCOUNT FOR FLUX TO/FROM STORAGE 
            IF(ISS .EQ. 0)THEN
...
C3      
      ...
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 GWF2LGR1CNVG
...
C1------CHECK IF ALL GRIDS ARE RUNNING 1-WAY COUPLED.  IF SO, RETURN
      CALL SGWF2LGR1PNT(2)            
      IF(MXLGRITER .LE. 1)THEN
        LGRCNVG = 1
        RETURN
      ENDIF

C2------IF REQUESTED, WRITE CONVERGENCE OF LGR ITERATIONS 
...
C
C3------NOT RUNNING 1-WAY COUPLED SO, CHECK CONVERGENCE OF EACH GRID
C3------IF NOT ON THE VERY FIRST LGR ITERATION OF THE FIRST STRESS
C3------PERIOD OF THE FIRST TIME STEP. (FORCE INITIAL LGR ITERATION)
      IF(LGRITER .GT. 1 .OR. KPER .NE. 1 .OR. KSTP .NE. 1)THEN
        DO L=2,NGRIDS
          CALL SGWF2LGR1PNT(L)            
          IF(ABS(HDIFFM) .GT. HCLOSELGR .OR. 
     &       ABS(FDIFFM) .GT. FCLOSELGR)THEN
             LGRCNVG = 0
             EXIT
          ELSE
            LGRCNVG = 1
      ...
      ENDIF
C
C4------IF MXLGRITER IS EXCEEDED, END ITERATION AND PRINT MESSAGE
      IF(LGRITER .GE. MXLGRITER)THEN
        LGRCNVG = 1
        IF(IOUTLGR .LT. 0)WRITE(*,515) 
        IF(IOUTLGR .GE. 0)WRITE(IOUT,515) 
      ENDIF
  515 FORMAT(1X,'MXLGRITER EXCEEDED ',/,
...
C5
      ...
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 GWF2LGR1PBD
      SUBROUTINE GWF2LGR1PBD(KSTP,KPER,IGRID,NGRIDS,NPCBEG,NPRBEG,
     1                       NPLBEG,NPCEND,NPREND,NPLEND,IBOTFLG,IBFLG,
     2                       PFLUX)
C     ******************************************************************
...
C     ------------------------------------------------------------------
      ...
      USE LGRMODULE,   ONLY:IUPBFSV,IUPBHSV
...
C1------CLEAR RATIN AND RATOUT ACCUMULATORS 
      ...
      ENDIF 
C
C2------LOOP THROUGH EACH PARENT FLUX B.C. CALCULATING FLOW.
C2------SEE IF FLOW IS POSITIVE OR NEGATIVE.
C2------FLOW RATE IS POSITIVE (RECHARGE). ADD IT TO RATIN.
C2------FLOW RATE IS NEGATIVE (DISCHARGE). ADD IT TO RATOUT.

      DO IPLAY=NPLBEG,NPLEND

C2A-----BACK FACE
        IRP=NPRBEG
        DO ICP=NPCBEG,NPCEND
          Q=PFLUX(ICP,IRP,IPLAY)
          QQ=Q
...
C2
      ENDDO
C2
C2E----INTERIOR BOTTOM FACE IF IT IS REFINED
      IF(IBOTFLG .EQ. 1)THEN
        IPLAY = NPLEND
        DO IRP = NPRBEG+1,NPREND-1
          DO ICP = NPCBEG+1, NPCEND-1
            Q=PFLUX(ICP,IRP,IPLAY)
            QQ=Q
            IF(Q .GT. 0) RATIN=RATIN+QQ
            IF(Q .LT. 0) RATOUT=RATOUT-QQ
...
C3------MOVE RATES, VOLUMES & LABELS INTO ARRAYS FOR PRINTING.
      ...
      VBNM(MSUM)=TEXT
C
C4------INCREMENT BUDGET TERM COUNTER(MSUM) IF ON THE LAST GRID.
      IF(IGRID .EQ. NGRIDS)MSUM=MSUM+1
C
C5------CALL ROUTINE TO OUTPUT PARENT B.C.'s FOR BFH IF REQUESTED
      IF(IUPBHSV .NE. 0 .OR. IUPBFSV .NE. 0)
     & CALL SGWF2LGR1BFHPOT(KSTP,KPER,NPCBEG,NPRBEG,NPLBEG,NPCEND,
     &                      NPREND,NPLEND,IBOTFLG,IBFLG,PFLUX)
C6------RETURN
      RETURN
      END
C***********************************************************************
C-----VERSION 1.0 15FEBRUARY2006 SGWF2LGR1BFHPOT
      SUBROUTINE SGWF2LGR1BFHPOT(KSTP,KPER,NPCBEG,NPRBEG,NPLBEG,NPCEND, 
     1                           NPREND,NPLEND,IBOTFLG,IBFLG,PFLUX)
C     ******************************************************************
...
C     ------------------------------------------------------------------
      ...
      USE LGRMODULE,   ONLY:ISCHILD,IUPBHSV,IUPBFSV
      DIMENSION PFLUX(NPCBEG:NPCEND,NPRBEG:NPREND,NPLBEG:NPLEND)
...
C1-------WRITE HEADER INFO, AND NODE LOCATIONS FOR INTERFACE CELLS
     ...
     &                      NBNODES,NBNODES,IBOTFLG,IBFLG,IUPBHSV 
          WRITE(IUPBHSV,400) NPLBEG,NPRBEG,NPCBEG
          WRITE(IUPBHSV,400) NPLEND,NPREND,NPCEND
        ENDIF
        IF(IUPBFSV .NE. 0) THEN
          WRITE(IUPBFSV,300) TEXTF,ISCHILD,NLAY,NROW,NCOL,SUM(NSTP),
     &                       NBNODES,NBNODES,IBOTFLG,IBFLG,IUPBHSV 
          WRITE(IUPBFSV,400) NPLBEG,NPRBEG,NPCBEG
          WRITE(IUPBFSV,400) NPLEND,NPREND,NPCEND
        ENDIF
C
        DO K=1,NLAY
          DO I=1,NROW
            DO J=1,NCOL
              IF(IBOUND(J,I,K) .EQ. IBFLG)THEN
                IF(IUPBHSV .NE. 0) WRITE(IUPBHSV,400) K,I,J
                IF(IUPBFSV .NE. 0) WRITE(IUPBFSV,400) K,I,J
              ENDIF
            ENDDO
          ENDDO
        ENDDO
        ...
        DO I=1,NROW
          DO J=1,NCOL
            IF(IBOUND(J,I,K) .EQ. IBFLG)THEN
              ...
              IF(IUPBFSV .NE. 0) WRITE(IUPBFSV,600) PFLUX(J,I,K)
            ENDIF
          ENDDO
        ENDDO
      ENDDO
...
C3------LOCATIONS IN THE PARENT GRID.
     ...
     &                      NBNODES,NPBNODES,NCPP,IBOTFLG,IBFLG,IUCBHSV 
          IB=0
          DO K=1,NLAY
            DO I=1,NROW
              DO J=1,NCOL
